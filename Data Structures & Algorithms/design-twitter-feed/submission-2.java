class Twitter {
    ...
    //userId--> ye jisoko follow kerta hai[following]
    HashMap<Integer,Set<Integer>> following;

    //user--->linkedlist of Tweets[head]
    ...
    public void postTweet(int userId, int tweetId) {
        Tweet tweet = new Tweet(tweetId, timer, tweetMap.get(userId));
        tweetMap.put(userId,tweet);
        timer++;
    }
    
    public List<Integer> getNewsFeed(int userId) {
        Set<Integer>fing = following.get(userId);
        if(fing==null) fing = new HashSet<>();
        fing.add(userId);
    ...
    //following---> ye jisko follow ker rha hai
    // followerId--jo follow ker rha hai, followeeId--jo follow ho rha hai,
    // ko follow ker rha hai
    public void follow(int followerId, int followeeId) {
